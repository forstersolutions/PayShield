...
- Dedicated regulated core backend scaffold, Dockerfile, compose manifest, and
  ...
  direct-deposit setup records store only masked paycheck-routing state.
  Commercial checkout intent records capture idempotent paid-access attempts
  before Stripe webhooks activate the household. The ledger migrations include
  deferred Postgres
...
- Core migration operations include a redacted, checksummed
  ...
  `PAYSHIELD_LEDGER_SCHEMA_VERIFIED_VERSION=0010` values only after the
...
- Applied Postgres ledger schema verified with
  ...
  `PAYSHIELD_LEDGER_SCHEMA_VERIFIED=true` and
  `PAYSHIELD_LEDGER_SCHEMA_VERIFIED_VERSION=0010` only from that successful
  verification output.
...
- Configured provider adapter environment:
  `PAYSHIELD_BAAS_PROVIDER`, `PAYSHIELD_BAAS_ADAPTER=http_json`,
  `PAYSHIELD_BAAS_API_BASE_URL`, and `PAYSHIELD_BAAS_API_KEY`.
