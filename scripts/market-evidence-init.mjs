...
function analyticsTemplate({ generatedAt, siteUrl }) {
  return {
    ...
    probe: {
      ...
      healthOk: false,
      landingPageRequested: false,
      paidTrafficReady: false,
      productInquiryApiSubmitted: false,
      receiptIdRecorded: false,
      receiptMode: "",
      sanitizedCampaignMetadataSubmitted: false,
      siteUrlMatchesExpected: false,
...
function commandsMarkdown({
  ...
  return [
    ...
    "```",
    "",
    "For Vercel Marketplace Upstash Redis capture, keep the Upstash template as the local placeholder before cutover. Generate final `receiver-evidence.json` only after the Upstash env vars are configured in Vercel Production, production is redeployed, and the required-capture smoke passes.",
    "",
    "```bash",
    ...
    "```",
    "",
    "If the selected durable path is Vercel Marketplace Upstash Redis, generate redacted receiver evidence from the live site and Upstash REST API:",
    "",
    "```bash",
